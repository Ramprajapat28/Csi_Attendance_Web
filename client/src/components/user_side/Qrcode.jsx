                ...
                .then((devices) => {
                    ...
                    html5QrCodeRef.current.start(
                        ...
                        (decodedText, decodedResult) => {
                            setData(decodedText);
                            localStorage.setItem('scannedData', decodedText);
                            if (scannerRunning && html5QrCodeRef.current) {
        ...
        return () => {
            ...
            if (scannerRunning && html5QrCodeRef.current) {
                html5QrCodeRef.current.stop()
                    ...
                    .catch((err) => {
                        console.warn('Stop on cleanup failed:', err);
                    });
            }
        };
    }, []);
    ...
    return (
        <div className="flex flex-col items-center justify-center w-screen h-[100dvh] gap-15 pt-[70px] pb-[30px]">
            <img
                ...
                className='h-[12px] absolute right-[15px] top-[25px]'
                alt="Cancel"
            />
            <div className="text flex flex-col items-center justify-center gap-0.5">
                <span className='font-bold text-lg'>Scan Code</span>
                <span className='font-medium text-gray-400 text-xs'>Scan QR Code to check securely</span>
            </div>
            <div
                id={qrCodeRegionId}
                className="w-[350px] h-[580px] rounded-[16px] p-0 m-0 overflow-hidden"
            />
            <button
                onClick={done}
                className='flex justify-center items-center rounded-lg text-sm font-medium gap-3 bg-[#1D61E7] text-white w-[350px] h-[48px] shadow-[0px_4px_4px_0px_#00000040] active:shadow-[0px_2px_1px_0px_#00000040] transition-all duration-100'
            >
                Done
                <img src="/src/assets/check.png" className='h-[15px] invert-100' alt="Check" />
            </button>
        </div>
    );
};
